...
public:   
    CartPoleGBDSolver(const CartPoleParams& params, std::unique_ptr<BaseMasterProblem> master, std::unique_ptr<BaseSubproblem> sub
        ) : BaseGBDSolver(params, std::move(master), std::move(sub)), params_(params) {
        }

    // Default constructor that uses GreedyMaster and BaseSubproblem
    explicit CartPoleGBDSolver(const CartPoleParams& params = CartPoleParams())
        : CartPoleGBDSolver(params, std::make_unique<GreedyMasterSolver>(params), std::make_unique<BaseSubproblem>(params)) {
        }
