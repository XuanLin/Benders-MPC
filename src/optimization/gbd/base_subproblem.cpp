...
void BaseSubproblem::updateInitialConditions(const VectorDyn& x0_new, const VectorDyn& h_theta_new) {
    ...
    try {
        // Update primal model bounds
        for (int i = 0; i < params_.nx; i++) {
            x0_vars_[i].set(GRB_DoubleAttr_LB, x0_new[i]);
            x0_vars_[i].set(GRB_DoubleAttr_UB, x0_new[i]);
            x0_infeas_vars_[i].set(GRB_DoubleAttr_LB, x0_new[i]);
            x0_infeas_vars_[i].set(GRB_DoubleAttr_UB, x0_new[i]);
        }
        
        for (int i = 0; i < params_.nc; i++) {
            h_theta_vars_[i].set(GRB_DoubleAttr_LB, h_theta_new[i]);
            h_theta_vars_[i].set(GRB_DoubleAttr_UB, h_theta_new[i]);
            h_theta_infeas_vars_[i].set(GRB_DoubleAttr_LB, h_theta_new[i]);
            h_theta_infeas_vars_[i].set(GRB_DoubleAttr_UB, h_theta_new[i]);
        }
